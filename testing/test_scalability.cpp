...
class Info {
  ...
  public:
  Info(const char *hostname, std::string request) : hostname(hostname), request(request) {}
  ~Info() {}
...
void * createClient(void * info) {
    Info *user = (Info *) info;
    const char * hostname = user->getHostname();
    std::string request = user->getRequest();
    int status;
    int socket_fd;
...
int main(int argc, char *argv[])
{
  ...
  Info *user = new Info(hostname, request_list[0]);

  auto start = std::chrono::high_resolution_clock::now();
  // 
  for(int i = 0; i < THREAD_NUM; ++i) {
    pthread_create(&thread[i], NULL, createClient, user);
  }
